...
def startBraveBrowserBuild() {
    BRAVE_BROWSER_BRANCH_ESCAPED = BRAVE_BROWSER_BRANCH.replace('/', '%2F')
    jobDsl(scriptText: """
        pipelineJob("pr-brave-browser-${BRAVE_BROWSER_BRANCH_ESCAPED}") {
    ...
    currentBuild.result = build(job: "pr-brave-browser-${BRAVE_BROWSER_BRANCH_ESCAPED}", parameters: params, propagate: false).result
}
